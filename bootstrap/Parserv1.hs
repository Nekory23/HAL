...
-- parse a Null
parseNull :: Parser ()
parseNull = Parser f where
    f str = case runParser (parseChar '(') str of
        Just (par1, str') -> case runParser (parseChar ')') str' of
            Just (par2, end) -> Just (() , end)
            Nothing -> Nothing
        Nothing -> Nothing

-- parse a Boolean
parseBool :: Parser Bool
parseBool = Parser f where
    f str = case runParser (parseChar '#') str of
        Just (par1, str') -> case runParser (parseChar 't') str' of
            Just (par2, end) -> Just (True, end)
            Nothing -> case runParser (parseChar 'f') str' of
                Just (par3, end') -> Just (False, end')
                Nothing -> Nothing
        Nothing -> Nothing